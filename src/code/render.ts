...
class Transformer {
    ...
    endPointToDeclaration(ep: IEndPoint) {
        ...
        this.declarations.push(
            ...[pathParams, bodyParams, queryParams, headerParams].reduce<ts.Declaration[]>((decs, curr) => {
...
export function Generator(server: Server, template: string, config: Schema2tsGeneratorConfig) {
    ...
    const code = render(ts.createSourceFile('generated.ts', result, ts.ScriptTarget.Latest, false)).replace(
        /@__JSDoc__ /g,
        '',
    )
