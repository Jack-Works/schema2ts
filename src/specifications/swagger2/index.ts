...
export function is(object: any, path: string): object is Swagger2.Document {
    ...
    if (!Swagger2.validateDocument(object)) {
        throw new TypeError(
            'Input file is not a valid Swagger2 specification, see http://bigstickcarpet.com/swagger-parser/www/index.html',
        )
    }
    return true
}
    ...
    switch (x.type as typeof x.type | 'object') {
        ...
        case 'object':
            if (x.schema && !x.schema.$ref) {
                return Swagger2SchemaToTypes(x.schema, doc)
            }
        ...
        default:
            ...
            if (key) {
                ...
                if (!doc.definitions) {
                    throw new TypeError('No definitions in schema but a ref is using it.')
                }
                return new Types.TypeReferenceType(k, Swagger2SchemaToTypes(doc.definitions[k], doc))
            }
            throw new TypeError(`Unknown type: Can not handle this type of object ${JSON.stringify(x)}`)
    }
}
function Swagger2ParameterToObject(
    parameters: Operation['parameters'],
    doc: Swagger2.Document,
): Types.ObjectOf | Types.TypeReferenceType {
    ...
    return new Types.ObjectOf(
        parameters.filter(x => x).map(param => {
            if (!param.name) {
                throw new TypeError(`${JSON.stringify(param)} has no name parameter.`)
            }
            return {
                key: param.name,
                ...
                value: param.schema
                    ? Swagger2SchemaToTypes(param.schema, doc)
                    : Types.shape(baseTypeMap[param.type as any]),
            }
        }),
    )
}
            ...
            for (const method in paths[url]) {
                ...
                if (p.responses) {
                    result = Object.keys(p.responses).map(
                        key =>
                            [parseInt(key, 10), Swagger2SchemaToTypes(p.responses[key], doc)] as [number, Types.Type],
                    )
                }
