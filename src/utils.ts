...
export function JSONSchemaToTypes(from: Schema, name = getJSONRefName(from)): Types.Type {
    ...
    if (from.allOf) {
        ...
        return JSONSchemaToTypes(newer, name)
        ...
        return new Types.And([JSONSchemaToTypes(newer, name), JSONSchemaToTypes(from.additionalProperties)])
    } else if (from.enum) {
    ...
    switch (type) {
        case 'array':
            if (Array.isArray(from.items)) {
                return new Types.ArrayOf(JSONSchemaToTypes(from.items[0], name))
            } else if (!from.items) {
                return new Types.Any()
            } else {
                return new Types.ArrayOf(JSONSchemaToTypes(from.items, name))
            }
        case 'object':
            ...
            const obj = new Types.ObjectOf(ofWhat)
            if (name) {
                return new Types.TypeReferenceType(name, obj)
            }
            return obj
        case 'string':
...
export const getJSONRefName = (x: Schema) => {
    return x && x.$ref ? x.$ref.replace('#/definitions/', '') : undefined
}
